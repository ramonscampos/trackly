    ...
    <DayPicker
      ...
      classNames={{
        ...
        caption_label: cn(
          ...
          captionLayout === 'label'
            ? 'text-sm'
            : 'flex h-8 items-center gap-1 rounded-md pr-1 pl-2 text-sm [&>svg]:size-3.5 [&>svg]:text-gray-400',
          defaultClassNames.caption_label
        ),
        table: 'w-full border-collapse',
        weekdays: cn('flex', defaultClassNames.weekdays),
        weekday: cn(
          'flex-1 select-none rounded-md font-normal text-[0.8rem] text-gray-400',
      ...
      components={{
        Root: ({ className: _cn, rootRef, ...rest }) => {
          return (
            <div
              className={cn(_cn)}
              data-slot="calendar"
        ...
        Chevron: ({ className: _cn, orientation, ...rest }) => {
          if (orientation === 'left') {
            return <ChevronLeftIcon className={cn('size-4', _cn)} {...rest} />
          }

          if (orientation === 'right') {
            return <ChevronRightIcon className={cn('size-4', _cn)} {...rest} />
          }

          return <ChevronDownIcon className={cn('size-4', _cn)} {...rest} />
        },
